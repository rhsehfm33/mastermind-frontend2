...
const actions = {
  LOGIN({ commit }, { email, password }) {
    return auth.login(email, password).then(({ accessToken }) => commit('LOGIN', { accessToken }));
  },
  // 이메일 중복 체크
  ...
  UPDATE_BOARD({ state, dispatch }, { id, title, bgColor }) {
    return board.update(id, { title, bgColor }).then(_ => dispatch('FETCH_BOARD', state.board.id));
  },
  DELETE_BOARD(_, id) {
  ...
  ADD_LIST({ state, dispatch }, { title, boardId, pos }) {
    return list.create({ title, pos, boardId }).then(_ => dispatch('FETCH_BOARD', state.board.id));
  },
  UPDATE_LIST({ state, dispatch }, { id, pos, title }) {
    return list.update(id, { pos, title }).then(_ => dispatch('FETCH_BOARD', state.board.id));
  },
  DELETE_LIST({ state, dispatch }, { id, pos, title }) {
    return list.destroy(id, { pos, title }).then(_ => dispatch('FETCH_BOARD', state.board.id));
  },
  ...
  ADD_CARD({ state, dispatch }, { title, pos, listId }) {
    return card.create({ title, pos, listId }).then(_ => dispatch('FETCH_BOARD', state.board.id));
  },
